...
int surface_area(box *b) {
    return (2 * b->len * b->width) + (2 * b->width * b->height) + (2 * b->height * b->len);
}
...
int total_paper(box *b) {
    return surface_area(b) + smallest_side_area(b);
}
